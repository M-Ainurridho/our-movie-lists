...
const Upcoming = async () => {
   ...
   return (
      <Section sectionId="upcoming" category="Upcoming">
         <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-4 justify-between">
            {upcoming.map(({ id, title, poster_path, release_date }: Movie, idx: number) => {
               ...
               return (
                  idx < 7 && (
                     <div className="movie-card relative" key={id}>
                        ...
                        <Link href={`/upcoming/${id}`} className="block text-sm mt-1.5">
