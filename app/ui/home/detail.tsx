...
const DetailMovie = ({ movies }: any) => {
   ...
   //             <Image width={150} height={250} src={`http://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="aspect-[4/6] object-cover rounded md:aspect-[4/5]" />
   ...
   //          {movies.map(
   //             (_: unknown, i: number) => i < 10 && <span key={i} className={clsx(`inline-block ${i === count ? "h-3 w-3 bg-white" : "h-2 w-2 bg-neutral-500"} rounded-full cursor-pointer`)} onClick={() => slidePaginate(i)}></span>
   //          )}
   //       </div>
